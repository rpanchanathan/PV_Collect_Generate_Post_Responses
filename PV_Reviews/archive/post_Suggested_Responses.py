...
def expand_all_reviews(iframe_locator):
    """Click More button until all reviews are loaded"""
    while True:
        try:
            ...
            break
    logger.info("Finished loading all reviews")

def post_replies_to_reviews(responses_file: str):
    """Post replies to unreplied Google reviews from existing Excel file"""
...
if __name__ == "__main__":
    responses_file = '/Users/rajeshpanchanathan/Documents/PythonWork/PV_Reviews/5stars_post_response_30Jan.xlsx'
    if os.path.exists(responses_file):
        post_replies_to_reviews(responses_file)
    else:
